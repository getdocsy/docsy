...
export function getSettings(): CliSettings {
  ...
  return {
    auth: {
      apiKey: env.DOCSY_API_KEY || systemFile.auth?.apiKey || defaults.auth.apiKey,
      apiUrl: env.DOCSY_API_URL || systemFile.auth?.apiUrl || defaults.auth.apiUrl,
      webUrl: env.DOCSY_WEB_URL || systemFile.auth?.webUrl || defaults.auth.webUrl,
    },
  };
}
...
function _loadDefaults(): CliSettings {
  return {
    auth: {
      apiKey: '',
      apiUrl: "https://app.getdocsy.com",
      webUrl: "https://getdocsy.com",
    },
  };
}
...
function _loadEnv() {
  return Z.object({
    DOCSY_API_KEY: Z.string().optional(),
    DOCSY_API_URL: Z.string().optional(),
    DOCSY_WEB_URL: Z.string().optional(),
  })
    .passthrough()
    .parse(process.env);
}
